...
unsigned char Read_scratchpad(unsigned char bus, unsigned char num)
{
    ...
    OWI_DetectPresence(bus);
    OWI_MatchRom(all_sensors[num].id, bus);    
  
    OWI_SendByte(DS18B20_READ_SCRATCHPAD, bus);
    for (i = 0; i<=8; i++){
      all_sensors[num].scratchpad[i] = OWI_ReceiveByte(bus);
    }
    
    if(OWI_CheckScratchPadCRC(all_sensors[num].scratchpad) != OWI_CRC_OK){
      return READ_CRC_ERROR;
    }
...
unsigned char Write_scratchpad(unsigned char bus, unsigned char num)
{
	...
	OWI_SendByte(0x7f, bus); // resolution is 12 bits	
